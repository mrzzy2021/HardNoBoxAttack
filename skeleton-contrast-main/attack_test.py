...
from dataset import get_attack_training_set, get_attack_validation_set
...
def traindata_extract_hidden(model, data_train):
    for ith, (ith_data, input_v1,input_v2, label) in enumerate(data_train):
...
def attackbykmeans( kmeans, model, data_eval):
    for ith, (ith_data,input_v1,input_v2, label) in enumerate(data_eval):
        # 参数设置
        ...
        input_v1 = input_v1.to(device).clone()
        input_v2 = input_v2.to(device).clone()
        ...
        features = en_hi.detach().cpu().numpy()
        Xte_Norm = preprocessing.normalize(features)
        en_hiv1 = model(input_v1, knn_eval=True)
        en_hiv2 = model(input_v2, knn_eval=True)
        en_hiv2 = en_hiv2.squeeze()
        en_hiv1 = en_hiv1.squeeze()
        features1 = en_hiv1.detach().cpu().numpy()
        Xte1_Norm = preprocessing.normalize(features1)
        features2 = en_hiv2.detach().cpu().numpy()
        Xte2_Norm = preprocessing.normalize(features2)
        # en_hiv2 = torch.nn.functional.normalize(en_hiv2, p=2, dim=1).unsqueeze(1)
        # en_hiv1 = torch.nn.functional.normalize(en_hiv1, p=2, dim=1).unsqueeze(1)
        ...
        precen1 = torch.tensor(precen1).cuda()
        precen2 = torch.tensor(pred2).cuda()
        ...
        for i in range(ep):
            ...
            label[1] = 1
            label[2] = 1
            label=label.unsqueeze(0).repeat(8,1).cuda()
            # loss = -1*nn.CrossEntropyLoss()(simloss, label)
            loss = -1 * nn.functional.binary_cross_entropy_with_logits(simloss, label)
            input.grad = None
            input.retain_grad()
            ...
            if i == ep - 1:
                np.savez_compressed('./Samples/NTU%dclucengcnaug.npz' % (ith),
            ...
            #             if sin[s] > 0.40 and update_prob < 0.04:
            #                 sig[s] = 1
            #             elif sin[s] > 0.35 and update_prob < 0.02:
            #                 sig[s] = 1
            #             elif sin[s] > 0.25 and update_prob < 0.01:
            #                 sig[s] = 1
            #     for s in range(len(label)):
            #         if sig[s] == 1:
            #             updates[s] = 0
        ...
        #     break
    return  input
def findpositivesamples(hi_train, model, input_tensor,label):
    ...
    for i in  range(len(label)):
        ...
        hardsamples = hi_train[hard_samples[0:3]]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = get_attack_training_set(opts)
    val_dataset = get_attack_validation_set(opts)
