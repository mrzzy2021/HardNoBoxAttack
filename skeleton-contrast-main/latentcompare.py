...
def attackbykmeans( model, data_eval):
    for ith, (ith_data, label) in enumerate(data_eval):
        ...
        ## gru-based attack
        path = 'C:/Motion/SMART/results/NTU/nobox/NTU1clucen91lp.npz'
        data = np.load(path)
        motions = data['clips']
        orMotions = data['oriClips']
        orMotions = np.array(orMotions)
        orMotions = torch.from_numpy(orMotions).cuda()
        ...
        with torch.no_grad():
            ...
            en_hi = en_hi.squeeze()
            features = en_hi.detach().cpu().numpy()
            en_hi1 = model( orMotions, knn_eval=True)
            en_hi1 = en_hi1.squeeze()
            features1 = en_hi1.detach().cpu().numpy()
        cos = nn.CosineSimilarity(dim=-1, eps=1e-1)
        simloss = cos(en_hi, en_hi1)
        ...
        for i in range(ep):
            ...
            en_hiatt = en_hiatt.unsqueeze(1).repeat(1,201,1)
            cos = nn.CosineSimilarity(dim=-1, eps=1e-1)
            ...
            if i % 25 == 0 or i == 0 or i == ep - 1:
                print(loss,  i)
                np.savez_compressed('./Samples/NTU%d0.01_370.npz' % (ith),
...
def findhardsamples(hi_train, model, input_tensor,label):
    ...
    for i in  range(len(label)):
        ...
        hardsamples = hi_train[indices[0:0+200]]
        tar_mot.append(hardsamples)
    tar_mot = torch.reshape(torch.cat(tar_mot, dim=0),(8,-1,2048))
