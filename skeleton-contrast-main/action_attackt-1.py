...
def velcal(input):
    ...
    vel = input[:,:,1:,:,:]-0.95*input[:,:,:-1,:,:] #S_t-S_t-1
    # St=input[:,:,1:,:,:]+1e-8 # S_t
    vel2 = 0.95 *input[:, :, 1:, :, :] - 0.95 * 0.95 *input[:, :, :-1, :, :]
    vel1 = torch.square(vel)
    vel1=torch.exp((-1*vel1)/2)# exp(-(S_t-S_t-1)^2)
    # vel1 = torch.exp((-1 * vel1) / 0.0002)  # exp((S_t-S_t-1)^2)
    vel1 = vel1*vel2*0.4
    # vel1 = torch.cat((c,vel1), dim=2)
    vel1 = torch.cat((vel1,c), dim=2)
    return vel1

def findnewcentre(en_hiatt,kmeans,precen2):
    prednew1 = []
    pos_index1=[]
    precen1 = np.zeros((8, 110, 256))
    posindex = np.zeros((8, 1, 256))
    en_hi1 = en_hiatt.clone()
    features1 = en_hi1.detach().cpu().numpy()
    Xte_Norm1 = preprocessing.normalize(features1)
    precen3 = Xte_Norm1.copy()
    cluscen = kmeans.cluster_centers_
    for i in range(8):
        a = np.expand_dims(precen3[i, :], 0).repeat(120, axis=0)
        idx = (np.sqrt(np.sum(np.square(a - cluscen), axis=-1)))
        a = np.random.randint(0, 1)
        max_index1 = np.argsort(idx)
        max_index1 = max_index1[10:]
        pos_index = max_index1[a]
        prednew1.append(max_index1)
        pos_index1.append(pos_index)
    pred1 = np.array(prednew1)
    pos_sam = np.array(pos_index1)
    for i in range(8):
        precen1[i, :] = cluscen[pred1[i], :]
        posindex[i, :] = cluscen[pos_sam[i], :]
    precen1 = torch.tensor(precen1)
    pos_sam = torch.tensor( posindex)
    precen2=precen2[:,1:,:]
    precen2=torch.cat((pos_sam,precen2), dim=1)
...
def  attackbykmeans( kmeans, model, data_eval):
    for ith, (ith_data, label) in enumerate(data_eval):
        ...
        for i in range(ep):
            ...
            model.eval()
            en_hiatt = model(input_ts, knn_eval=True)
            ...
            # buffer = cgs
            ## 计算平均速度
            vel = velcal(input_ts).cuda()
            # change = cgs[:, :, 1:,:, :] - cgs[:, :, :-1, :, :]
            change = cgs[:, :, 1:, :, :]
            # change = cgs[:, :, :-1, :, :]
            changes = torch.cat(( change,c),dim=2)
            # cgs = cgs + momentum*1+1*(past - cgs)-0.5*changes
            ...
            if i % 25 == 0 or i == 0 or i == ep - 1:
                print(loss,  i)
            if   i == ep-1:
                np.savez_compressed('./Samples/NTU%dgcnSt-1eval0.95.npz' % (ith),
