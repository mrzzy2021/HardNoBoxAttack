...
def attackbykmeans( kmeans, model, data_eval):
    for ith, (ith_data, label) in enumerate(data_eval):
        ...
        for i in range(ep):
            ...
            cgs = input.grad
            cgs = cgs / torch.mean(torch.abs(cgs), dim=(1,2), keepdim=True)
            change=cgs[:,1:,:]-cgs[:,:-1,:]
            changes = torch.cat((c,change),dim=1)
            ...
            if i % 25 == 0 or i == 0 or i == ep - 1:
                print(loss,  i)
            if  i == ep - 1:
                np.savez_compressed('./Samples/NTU%dgruM2.npz' % (ith),
