...
class Feeder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        flip_prob  = random.random()
        if flip_prob < 0.5:
                 data_numpy_v3 = augmentations.joint_courruption(data_numpy)
        else:
                 data_numpy_v3 = augmentations.pose_augmentation(data_numpy)

        flip_prob = random.random()
        if flip_prob < 0.5:
                 data_numpy_v4 = augmentations.joint_courruption(data_numpy)
        else:
                 data_numpy_v4 = augmentations.pose_augmentation(data_numpy)
        ...
        if  self.input_representation == "seq-based":
             ...
             #View3
             # input_v3 = data_numpy_v2.transpose(1,2,0,3)
             # input_v3 = input_v3.reshape(-1,150).astype('float32')
             # #View 4
             # input_v4 = data_numpy_v2.transpose(1,2,0,3)
