...
parser.add_argument('--pretrained', default='./checkpoints/checkpoint_450.pth.tar ', type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # use the cluster results as the new label
        pred, label_train = clustering_knn_acc(model, train_loader, val_loader)
        pred = torch.tensor(pred)


        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, pred)
        ...
        }, 1, filename=args.finetune_skeleton_representation + '_checkpoint.pth.tar')
...
def clustering_knn_acc(model, train_loader, eval_loader):
    model.eval()
    ...
    pred= kmeanstraining(hi_train, label_train, nc=60)

    return pred,  label_train
...
def kmeanstraining(data_train, labels,  nc=60):
    ...
    pred = kmeans.predict(Xtr_Norm)

    return pred

def train(train_loader, model, criterion, optimizer, epoch, args, pred):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        pred1 = pred[i*len(target):(i+1)*len(target)]
        pred1 = pred1.type(torch.LongTensor).cuda(args.gpu, non_blocking=True)

        # compute output
        output = model(images)
        loss = criterion(output, pred1)

        # measure accuracy and record loss
        acc1, acc5 = accuracy(output, pred1, topk=(1, 5))
...
def attack(val_loader, model, criterion, args):
    ...
    for i, (input_tensor, target) in enumerate(val_loader):
        ...
        for p in range(ep):
            ...
            classLoss = torch.nn.CrossEntropyLoss()(output, predictedLabels)
            input.grad = None
            input.retain_grad()
            ...
            np.savez_compressed('./Samples/NTU%dfit.npz' % (p),
