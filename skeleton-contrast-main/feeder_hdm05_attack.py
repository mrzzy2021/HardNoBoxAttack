...
class Feeder(torch.utils.data.Dataset):
    ...
    def load_data(self, mmap):
        ...
        if mmap:
            ...
            self.data_all = np.load(self.data_path)
            self.data = self.data_all['clips']
            self.label = self.data_all['classes']
    ...
    def __getitem__(self, index):
        ...
        data_numpy = np.reshape(data_numpy, (self.C, 25, 3))
        data_numpy = data_numpy[..., np.newaxis]
        second_body = np.zeros((60, 25, 3, 1))
        data_numpy = np.concatenate((data_numpy, second_body), axis=-1)
