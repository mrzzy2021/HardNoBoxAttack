...
# parser.add_argument('--pretrained', default='./checkpoints/interskeleton_seq_based_graph_based/checkpoint_0350.pth.tar', type=str,
#                     help='path to moco pretrained checkpoint')
parser.add_argument('--pretrained', default='./checkpoints/checkpoint_test120_0450.pth.tar', type=str,
                    help='path to moco pretrained checkpoint')
parser.add_argument('--finetune-dataset', default='ntu120', type=str,
                    help='which dataset to use for finetuning')

parser.add_argument('--protocol', default='cross_setup', type=str,
...
def  attackbykmeans( kmeans, model, data_eval):
    for ith, (ith_data, label, number_of_frame) in enumerate(data_eval):
        ...
        path = '../../ARmodel120/NTU{}CVval120.npz'.format(ith)
        data = np.load(path)
        Ar1 = data['AR1']
        Ar2 = data['AR2']
        ...
        for i in range(ep):
            ...
            if   i == ep-1:
                np.savez_compressed('../../NTU120/testS2MI-FGSM/NTU%d_120_06.npz' % (ith),
