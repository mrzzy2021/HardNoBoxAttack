...
def attackbykmeans(model, data_eval):
    for ith, (ith_data,input_v1,input_v2,label) in enumerate(data_eval):
        ...
        # # 按排序找到所有中心
        # for i in range(len(label)):
        #     a = np.expand_dims(precen2[i, :], 0).repeat(60,axis=0)
        #     idx = (np.sqrt(np.sum(np.square(a - cluscen), axis=-1)))
        #
        #     max_index1 = np.argsort(idx)
        #     max_index1 = np.delete(max_index1, (0), axis=0)
        ...
        for i in range(ep):
            ...
            # en_hiatt = en_hiatt.unsqueeze(1).repeat(1,91,1)
            features = torch.cat(
                [en_hiatt.unsqueeze(1), en_hiv1.unsqueeze(1), en_hiv2.unsqueeze(1), en_hi.unsqueeze(1)], dim=1)
            loss_contrast = contrast_criterion(features)
            loss=loss_contrast
            ...
            if i % 25 == 0 or i == 0 or i == ep - 1:
                print(loss,  i)
                np.savez_compressed('./Samples/NTU%daug.npz' % (ith),
...
def clustering_knn_acc(model, train_loader, eval_loader, criterion, num_epoches=400, middle_size=125, knn_neighbours=5):
    ...
    result = attackbykmeans( model, eval_loader)
