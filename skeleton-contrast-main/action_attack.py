...
parser.add_argument('--pretrained', default='./checkpoints/checkpoint_half_0450.pth.tar', type=str,
...
def update(input, X, eps):
    ...
    input = torch.where(input > X + 3*eps, X + 3*eps, input)
    input = torch.where(input < X - 3*eps, X - 3*eps, input)
    input[:, :, :, (0, 1, 2, 3, 12, 16, 20)] = head
    # input = input.permute(0, 2, 3, 1, 4)
    # input = torch.reshape(input, (input.shape[0], 3,300, 25, 2))
...
def aug(input):
    ...
    return input
def velcal(input, means):
    input = input.detach().cpu()
    means = means.detach().cpu()
    c = torch.zeros((8, 3, 1, 25, 2))
    # input = torch.nn.functional.normalize(input, p=2, dim=2)
    ASt_1=means*input
    vel = input[:, :, 1:, :, :]-ASt_1[:, :, :-1, :, :] #S_t-AS_t-1
    # St=input[:,:,1:,:,:] # S_t
    vel1 = torch.square(vel)
    vel2=torch.exp((-1*vel1)/(2*0.01))# exp(-(S_t-S_t-1)^2/2sigma^2)
    # # # vel1 = torch.exp((-1 * vel1) / 0.0002)  # exp((S_t-S_t-1)^2)
    vel3 = vel2*means[:, :, :-1, :, :]*vel*400 #exp(-(S_t-S_t-1)^2/2sigma^2)*(St)
    vel4 = torch.cat((vel3,c), dim=2)
    # vel4 = torch.cat((c, vel), dim=2)
    vel4[np.isnan(vel4)] = 0.5

    return vel4
def rcal(inputnp, num_of_frame, label, c, r, d):
    tvrge = importr("tvReg")  # library R package
    pandas2ri.activate()
    Num = 300 // num_of_frame[r] + 2
    for i in range(25):
        for k in range(3):
...
def autore(input, num_of_frame, label ):
    ...
    results= Parallel(n_jobs=8)(
        delayed(rcal)(inputnp, num_of_frame, label, c, r, d)
        for r in range(8))
    ...
    #             inputnp1 = inputnp1.tolist()
    #             inputnp1 = robjects.FloatVector(inputnp1)
    #             result = tvrge.tvAR(inputnp1, p=1, type="none", est="ll")
    #
    #             coef = result.rx2("coefficients").T
    #             coef = np.squeeze(coef)
    #             coef = np.pad(coef, (0, 1), 'constant', constant_values=(0, 0))
    #             coef = np.tile(coef, Num)
    #             coef = coef[:300]
    #             c[r, k, :300, i, 0]=coef
    #     if label[r] >= 50:
    #         for i in range(25):
    ...
    #                 inputnp2 = inputnp2.tolist()
    #                 inputnp2 = robjects.FloatVector(inputnp2)
    #                 result = tvrge.tvAR(inputnp2, p=1, type="none", est="ll")
    #
    #                 coef = result.rx2("coefficients").T
    #                 coef = np.squeeze(coef)
    #                 coef = np.pad(coef, (0, 1), 'constant', constant_values=(0, 0))
...
def vrcal(inputnp, num_of_frame, label, c, r, d):
    tvrge = importr("tvReg")  # library R package
    # pandas2ri.activate()
    Num = 300 // num_of_frame[r] + 2
    for i in range(25):
            inputnp1 = inputnp[r, :, :num_of_frame[r], i, 0]
            inputnp1 = np.array(inputnp1)
            nr, nc = inputnp1.shape
            inputnpvec = robjects.FloatVector(inputnp1.reshape((inputnp1.size)))
            inputnpvec1 = robjects.r.matrix(inputnpvec, nrow=nc, ncol=nr)
            result = tvrge.tvAR(inputnpvec1, p=1, type="none", est="ll")

            coef = result.rx2("coefficients")
            coef = np.squeeze(coef)
            coef0 = coef[0, :]
            coef0 = np.pad(coef0, (0, 1), 'constant', constant_values=(0, 0))
            coef0 = np.tile(coef0, Num)
    ...
    if label[r] >= 50:
        for i in range(25):
            for k in range(3):
                ...
                d[k, :, i, 1] = coef1
    return c, d
def vautore(input, num_of_frame, label ):
    inputnp = input.detach().cpu().numpy()
    num_of_frame = num_of_frame.detach().cpu().numpy()
    label =label.detach().cpu().numpy()
    c = np.zeros((3, 300, 25, 2))
    ...
    # 并行
    results= Parallel(n_jobs=8)(
        delayed(vrcal)(inputnp, num_of_frame, label, c, r, d)
        for r in range(8))
    results = np.asarray(results)
    c = results[:, 0, :, :, :, :].squeeze()
    d = results[:, 1, :, :, :, :].squeeze()
...
def  attackbykmeans( kmeans, model, data_eval):
    for ith, (ith_data, label, number_of_frame) in enumerate(data_eval):
        ...
        for i in range(ep):
            ...
            # cgs = cgs + momentum*1
            # cgs = cgs + momentum * 1 + vel * changes + vel1 * changes1
            # cgs = cgs + momentum * 1
            ...
            if i % 25 == 0 or i == 0 or i == ep - 1:
                print(loss,  i)
            # if i<=20:
            #     np.savez_compressed('./Samples/NTU%dgcni051%d.npz' % (ith,i),
            #                         clips=ith_data.cpu().detach().numpy(), oriClips=input.cpu().detach().numpy(),
            #                         labels=label.cpu().detach().numpy())
            # if i >20:
            #     break
            if   i == ep-1:
                np.savez_compressed('./SamplesI-FGSM/NTU%dgcni01.npz' % (ith),
                                    clips=ith_data.cpu().detach().numpy(), oriClips=input.cpu().detach().numpy(),
                                    labels=label.cpu().detach().numpy())
            ...
            #             if sin[s] > 0.35 and update_prob < 0.05:
            #                 sig[s] = 1
            #             elif sin[s] > 0.25 and update_prob < 0.03:
            #                 sig[s] = 1
            #             elif sin[s] > 0.15 and update_prob < 0.01:
            #                 sig[s] = 1
            #     for s in range(len(label)):
            #         if sig[s] == 1:
            #             updates[s] = 0
